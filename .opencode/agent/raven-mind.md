...
- **Convención de Etiquetado**: Cada archivo fuente nuevo o modificado en `munin/` (`# tags: [core, mcp, ...]`), en `app/` (`// tags: [ui-component, ai-sdk-v5, tanstack-query, indexeddb, ...]`) y CSS (`/* tags: [...] */`) debe comenzar en la primera línea con un encabezado de etiquetas (tags) para facilitar las búsquedas globales de agentes y ripgrep/grep, tal como se especifica en las skills `munin-management` y `munin-frontend`.
