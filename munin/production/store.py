...
class ProductionStore:
    ...
    def for_settings(cls, settings: Any, *, master_key: bytes) -> ProductionStore:
        ...
        This deliberately opens a fresh connection per operation, matching the
        existing Turso adapter. Transactions below issue explicit SQL `BEGIN`
        and `COMMIT`; callers never depend on autocommit context-manager magic.
        """
    ...
    def migrate(self) -> None:
        with self._transaction() as conn:
            ...
            if row:
                if row["checksum"] != MIGRATION_CHECKSUM:
    ...
    def claim_next_run(self, *, worker_id: str, lease_seconds: int = 60) -> dict[str, Any] | None:
        with self._transaction() as conn:
            row = conn.execute("SELECT * FROM agent_runs WHERE state='queued' ORDER BY created_at_ms,id LIMIT 1").fetchone()
            if not row:
                return None
            now = _now_ms()
            ...
            changed = conn.execute(
                "UPDATE agent_runs SET state='running',lease_worker_id=?,lease_token=?,lease_expires_at_ms=?,fencing_epoch=?,state_version=state_version+1,updated_at_ms=? WHERE id=? AND state='queued' AND state_version=?",
                (worker_id[:160], lease_token, now + max(5, lease_seconds) * 1000, next_epoch, now, row["id"], row["state_version"]),
            )
            if int(changed.rowcount) != 1:
                return None
            conn.execute("UPDATE messages SET status='running',updated_at_ms=?,version=version+1 WHERE id=?", (now, row["assistant_message_id"]))
            self._append_event(conn, run_id=row["id"], kind="run.claimed", payload={"worker_id": worker_id, "fencing_epoch": next_epoch})
            claimed = conn.execute("SELECT * FROM agent_runs WHERE id=?", (row["id"],)).fetchone()
            result = self._run_dict(claimed)
    ...
    def renew_lease(self, *, run_id: str, lease_token: str, lease_seconds: int = 60) -> bool:
        with self._transaction() as conn:
            now = _now_ms()
            changed = conn.execute(
                "UPDATE agent_runs SET lease_expires_at_ms=?,updated_at_ms=? WHERE id=? AND state='running' AND lease_token=? AND lease_expires_at_ms>=?",
                (now + max(5, lease_seconds) * 1000, now, run_id, lease_token, now),
            )
    ...
    def append_operator_guidance(self, *, actor_id: str, run_id: str, guidance: str) -> dict[str, Any]:
        ...
        with self._transaction() as conn:
            ...
            if not run:
                raise KeyError(run_id)
            self._require_participant(conn, actor_id=actor_id, conversation_id=run["conversation_id"])
            event = self._append_event(conn, run_id=run_id, kind="operator.guidance", payload={"guidance": redact_text(guidance)[:4_000]}, actor_id=actor_id)
            self._audit(conn, actor_id=actor_id, action="run.guidance", resource_type="run", resource_id=run_id, outcome="success")
            return event
