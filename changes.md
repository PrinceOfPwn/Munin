...
## 2026-08-03 â€” Valravn mesh: integrate Burp DAST surface as `valravn/`
...
- New repo subtree `valravn/` integrating the Burp Suite agentic DAST
  ...
  upstream attribution retained in `valravn/NOTICE` per Â§4(d).
...
- Removed `valravn/mcp-server/.claude/skills/` (11 English snapshots older
  than the Chinese top-level `.claude/skills/` â€” duplicate-maintenance hazard).
...
- New opencode skill `.opencode/skills/valravn-diagnostic/SKILL.md` â€” operator
...
- Soul: rewrote `soul/valravn.md` from Chinese to English YAML-frontmatter form,
  ...
  and `principles.md Â§2` campaign-loop integration as before.
- Workflow `valravn-smoke.yml` extended with a second job
  ...
  Apache-2.0 Â§4(d) attribution retention in `NOTICE`, assert `.gitignore`
...
- Burp Edition compatibility, deployment on hosts without Burp, and behavioral
  ...
  operator's host where Burp + Java 21 are installed â€” `valravn-diagnostic`
