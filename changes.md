...
## 2026-08-03 — PLAN-5 of Issue #32: renderer registry + sandboxed HTML preview (5A/5B/5C/5D)

Sixth slice of PLAN-5 executed in worktree `munin-pr6` (branch `raven-mind/pr-6-renderer-registry`, stacked on PLAN-6 @ `6eb47c6`). Cards implemented in dependency order: 5B (backend guard + registry) → 5A (sandboxed preview) → 5D (renderer set incl. mermaid + IOC table) → 5C (Playwright security lab). Operator commits (no git actions taken by the agent).

**5A — Sandboxed HTML preview.** New `app/src/lib/sandboxContract.ts` is the single source of truth for the iframe contract: `SANDBOX_ATTRIBUTES = "allow-scripts"` (never `allow-same-origin` — opaque origin), `SANDBOX_CSP = "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:;"`, and `buildSandboxedDocument`. `SandboxedPreview.tsx` renders the hardened iframe (memoized, srcdoc stable via `useMemo`), logs via `logError` and renders a safe fallback card for invalid/empty payloads — defense in depth on top of registry-side validation.

**5B — artifact/sandboxed-html registry + server guard.** Frontend: `app/src/components/chat/blocks/registry.ts` (plain `.ts`, `createElement`-based) — zod schemas (`artifactContentSchema`, `sandboxedHtmlArtifactSchema` with literal discriminator, `iocTableSchema`), `ARTIFACT_MEDIA_TYPES` (11), `normalizeMediaType`, `registerBlockRenderer`/`lookupBlockRenderer`, `BlockRendererFor` + `BlockRendererErrorBoundary` + `BlockFallbackCard`, idempotent builtin registration. `ArtifactPart.tsx` now consults the registry for body-bearing artifacts (optional `content`/`previewUrl`/`downloadUrl` props). Backend: `munin/production/redaction.py` gains `SANDBOXED_HTML_MEDIA_TYPE`, `precheck_sandboxed_html` (raises `ValueError` for external `<script src>` — http/https/protocol-relative), `sanitize_sandboxed_html` (strips external src attrs on other elements, idempotent), `sanitize_artifact_content` dispatcher; `store.py` `_insert_artifact` calls it before `redact_text`. The guard is NOT bypassed by `MUNIN_REDACTION_MODE=off` (documented).

**5C — Playwright security lab.** `app/playwright.config.ts` (testDir `tests/e2e`, chromium project, `PLAYWRIGHT_BASE_URL` default `http://127.0.0.1:3000`, traces on first retry), `app/tests/fixtures/malicious_html_payloads.ts` (6 adversarial payloads writing `data-*` outcome markers), `app/tests/e2e/sandboxed_iframe_security.spec.ts` (11 tests, self-contained — no server needed; imports the contract constants from `sandboxContract.ts` via relative path). Verified live in Chromium 151 headless shell: 11/11 passed. Assertions cover: exact sandbox/CSP strings, zero `/api/*` requests, zero external-host requests, parent-DOM access → `SecurityError`, cookie/storage access → `SecurityError`, inline scripts still run (control).

**5D — Initial renderer set.** All 11 media types registered: markdown (Markdown), text/code/diff/json/csv/table (CommandOutputPart), mermaid + mermaid-alt (new `MermaidPart.tsx` — lazy `import("mermaid")`, `securityLevel: "strict"`, dark theme, `logError` + fenced-code fallback), sandboxed-html (SandboxedPreview), ioc-table (new `IocTablePart.tsx` — pure `classifyIoc`/`parseIocContent`/`filterIocRows` exported, search filter, `Badge` variants, canonical IPv6 regex incl. `::` compression). Barrel `parts/index.ts` updated with full prop types. Registry routing for the full set is covered by vitest (a browser `renderers_set` spec would need the app server, so it stays unit-covered — documented deviation).

**Validation (local):** backend `py_compile` 0, `ruff check --select F` 0, pytest `tests/test_sandboxed_html_registry.py` 17 passed + guidance lifecycle regression 9 passed (1 pre-existing warning); frontend `tsc --noEmit` 0 errors, `npm test` 116 passed (5 files, incl. 28 registry + 12 sandboxed-preview + 17 IOC-table tests), `npm run lint` 0 errors (1 pre-existing `aiChat.ts` exhaustive-deps warning), Playwright e2e 11 passed in real Chromium. Note: on this Windows host pytest needs `--basetemp=C:\Users\Emi\AppData\Local\Temp\opencode\pytest-pr5` because the default `pytest-of-Emi` basetemp is locked by an unrelated user process.

**Dependencies:** `mermaid@11.16.0` + `@playwright/test@1.62.1` added (decision: end-to-end mermaid, not vendored fallback). Lockfile regenerated with `npx -y npm@10 install --package-lock-only --ignore-scripts` (npm@10/Node 22 CI parity; local npm is 11.6.2) — `lockfileVersion` 3, 0 vulnerabilities. Playwright browser (`chromium-headless-shell`) installed locally; specs are CI-ready but wiring GitHub Actions is operator-owned (workflows off-limits).

## 2026-08-03 — PLAN-6 of Issue #32: artifact read-model + run detail (6A/6B/6C/6D/6E)

Fifth slice of PLAN-6 executed in worktree `munin-pr5` (branch `raven-mind/pr-5-artifacts-readmodel`, stacked on PLAN-4 @ `a742758`). All five cards implemented in dependency order: 6A → 6B → 6C → 6D → 6E. Local validation: `ruff check --select F` 0, py_compile 0, `pytest` on the six new files (26 passed) + guidance lifecycle regression (9 passed, 35 total, 16.95s). No frontend files touched; no new dependencies. Operator commits (no git actions taken by the agent).

- PR-6A `store.py`: `_FASE3_OPTIONAL_COLUMNS` extended (inside the existing Fase-3 mechanism, per the hard rule) with `conversation_artifacts` `renderer TEXT`, `version INTEGER NOT NULL DEFAULT 1`, `provenance TEXT`, `preview_url TEXT`, `download_url TEXT`; `_insert_artifact`/`add_artifact` accept the five new kwargs (16-column INSERT; `version=max(1,int(version))`; strings capped — renderer 80, provenance 500, urls 2000); new index `idx_conversation_artifacts_conv_created` on `(conversation_id, created_at_ms)` added to `_FASE3_DDL`. New read methods (read-only, deterministic, content omitted): `list_artifacts_for_conversation` (chronological oldest-first per the 6B card; KeyError for unknown conversation BEFORE the participant check so the handler's 404 branch is reachable) and `list_artifacts_for_run` (oldest first; KeyError first, then participant 403).
- PR-6A `asgi.py`: `conversation_artifacts` handler + `GET /api/chat/{conversation_id}/artifacts` route; docstring updated (provider-profiles bullet restored, runs bullet added).
- PR-6B `runs.py`: `run_artifacts` handler + `GET /api/runs/{run_id}/artifacts` (oldest first, metadata only, KeyError→404 / PermissionError→403); docstring covers PR-6B/6C.
- PR-6C `store.py`: `get_run_detail_readmodel(actor_id, run_id)` — EXACTLY the ten contract keys (`run_id`, `state`, `aggregated_tools`, `activities`, `commands`, `agents`, `approvals`, `guidance`, `artifacts`, `summaries`), pure SQL over the durable read-model tables, no provider calls, deterministically ordered (aggregated_tools grouped by tool_name with call_count/by_state; activities = `operational_summary` reasoning events; commands = tool_calls matched via `tool.output` tool_call_id with chunk_count/unique_streams; approvals from `human_requests` with resolution derived from `response.choice`; guidance from `run_guidance_queue`; summaries from `conversation_summaries`). `MuninStore` gained the matching hot-first override beside `get_run_detail_for_actor`; `runs.py` route `GET /api/runs/{run_id}/detail`.
- PR-6D `chat.py`: `_envelope_from_event` normalization block for `subagent.*` durable events — REUSES the existing vocabulary (decision: no new envelope kinds, no frontend contract changes; `subagent.queued/started/spawn`→`subagent_started` state "started", all others→`subagent_state` with truthful state), `subagent_id` from `subagent_run_id` fallback, `name` from `profile_id` fallback, envelope dropped when no id. Verified against the BFF contract (`DOTTED_KIND_MAP` already maps `subagent.queued→subagent_started`, `subagent.state→subagent_state`; `translator.ts` already renders `data-subagent`). Live producers confirmed: `munin/core/runtime_adapter.py::translate_events` (activity) and `munin/core/middleware/progress_emit.py` (tool lifecycle only) — `handoff_tools.py`/`send_workers.py`/`kernel.py` emit no envelopes, so `subagent_handoff`/`fanout_branch` remain vocabulary without runtime producers (documented in the test module). `create_subagent_run` (store.py:1691) remains the only durable subagent writer, still zero callers in v1.0.0.
- PR-6E `tests/test_readmodel_invariants.py`: 1000 consecutive authenticated GETs across the four read endpoints (`/api/artifacts/{id}`, `/api/chat/{conv}/artifacts`, `/api/runs/{run}/artifacts`, `/api/runs/{run}/detail`) with `sqlite3.Connection.set_trace_callback` armed on every connection (via `_connection_factory` wrapper) asserting ZERO INSERT/UPDATE/DELETE/ALTER/CREATE/DROP/REPLACE; row counts + rowid/id-length checksums unchanged across the nine read-model tables; run-detail payload byte-identical across the loop. Auth session pre-touched before arming so `authenticate`'s idle-refresh `UPDATE auth_sessions` (line ~963, `_SESSION_TOUCH_INTERVAL_MS` default 120s) cannot fire mid-loop.
- Tests added: `tests/test_artifacts_store.py` (Fase-3 column migration incl. legacy 11-column DB upgrade path with version=1 DEFAULT fallback + post-upgrade 16-column write, add/get round-trip with rich metadata, defaults, complete_run fenced-extraction regression), `tests/test_asgi_artifacts.py` (rich-metadata GET, download attachment, 404/403), `tests/test_artifacts_endpoints.py` (conversation chronological oldest-first, run oldest-first, 404/403 both), `tests/test_run_detail_endpoint.py` (exact ten keys + every section via `_rich_run` fixture, byte-identical determinism, 404/403, no-provider guard with monkeypatched `reveal_provider_key`/`list_provider_profiles`), `tests/test_sse_workflow_events.py` (envelope normalization cases, drop-without-id, durable `subagent.queued` round-trip, activity reuse via `reasoning_by_eid`, `translate_events` never-invents guard with `AIMessageChunk`).
- Deviations from cards: 6D reuses the existing `subagent_started`/`subagent_state`/`activity` envelope vocabulary instead of introducing new kinds (agents.py no longer exists — removed in PR-0 — and the BFF contract already covers the mapping; documented in the test module docstring); 6A schema changes landed inside the existing `_FASE3_*` mechanism per the hard rule; unknown conversation on the conversation-artifacts endpoint returns 404 (existence checked before participation) for consistency with the run-scoped endpoints.
- TODOs: operator commit of this branch, push, and stacked PR (base raven-mind/pr-4-part-hardening). The PR-2F duplicate-export fix is already in this branch's base chain (PR-44 CI green), so the build gate applies as usual. No frontend work in this slice — the GUI consumes the new endpoints as-is.
