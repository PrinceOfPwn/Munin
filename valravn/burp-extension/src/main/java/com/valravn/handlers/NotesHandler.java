...
import com.valravn.store.FindingsStore;
...
public class NotesHandler extends BaseHandler {
    ...
    private String describeEndpointMismatch(int index, String endpoint) {
        return com.valravn.util.EvidenceMatch.describeMismatch(api, index, endpoint);
    }
    ...
    private void handleSave(HttpExchange exchange) throws Exception {
        ...
        if (com.valravn.store.FindingsStore.isNeverSubmit(vulnType, title)) {
        ...
        if (hasCollab) {
            ...
            // Cross-validate against the shared CollaboratorPool. Returns null
            // when Collaborator is unavailable (Community Edition) — in that
            // case fall through to length-only validation rather than reject.
            Boolean known = com.valravn.collaborator.CollaboratorPool
                .hasInteraction(api, id);
            if (Boolean.FALSE.equals(known)) {
                sendError(exchange, 400,
                    "evidence.collaborator_interaction_id not observed in pool: " + id,
                    "collaborator_id_unknown",
                    "Run generate_collaborator_payload(), trigger the probe, then poll "
        ...
        if (com.valravn.store.FindingsStore.requiresReproductions(vulnType)) {
        ...
        // Python's _deduplicate_finding handles persistent dedup in .valravn-intel/.
        // This catches the in-memory case where the same finding is re-posted
        // (e.g. operator re-runs save_finding after a transient failure).
        String endpoint = (String) body.get("endpoint");
        Map<String, Object> existing = store.findExistingByKey(endpoint, vulnType, title);
        if (existing != null) {
            sendJson(exchange, JsonUtil.object(
                "id", existing.get("id"),
                "action", "deduped",
                "existing", existing,
                "hint", "Same (endpoint, vuln_type, title) already saved at id="
                    + existing.get("id") + ". Use DELETE /api/notes/findings/{id} to remove first if you mean to replace."
            ));
            return;
        }
