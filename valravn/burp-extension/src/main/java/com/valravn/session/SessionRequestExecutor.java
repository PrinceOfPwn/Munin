...
import static com.valravn.http.HttpResponses.sendJson;
import static com.valravn.http.HttpResponses.sendError;
...
public final class SessionRequestExecutor {
    ...
    private static final int MAX_RESPONSE_SIZE = com.valravn.server.ResponseLimits.MAX_RESPONSE_BODY;
    ...
    public void handle(HttpExchange exchange, Map<String, Object> body, SessionStore store) throws Exception {
        ...
        synchronized (session) {
            ...
            if (analyzeFlag instanceof Boolean b && b && result.response() != null) {
                ...
                analysis.put("tech_stack", com.valravn.analysis.TechStackDetector.detect(resp));
                analysis.put("injection_points", com.valravn.analysis.InjectionPointDetector.detect(req, resp));
                analysis.put("parameters", com.valravn.analysis.ParameterExtractor.extract(req));
                String contentType = resp.headerValue("Content-Type") != null ? resp.headerValue("Content-Type") : "";
                if (contentType.contains("html")) {
                    String bodyStr = resp.bodyToString();
                    analysis.put("forms", com.valravn.analysis.FormExtractor.extract(bodyStr));
                    analysis.put("endpoints", com.valravn.analysis.EndpointExtractor.extract(bodyStr, req.url()));
                }
                if (contentType.contains("javascript") || contentType.contains("html")) {
                    analysis.put("secrets", com.valravn.analysis.JsSecretExtractor.extract(resp.bodyToString()));
                }
    ...
    public HttpRequestResponse send(Session session, Map<String, Object> params) {
        ...
        try {
            ...
            HttpRequestResponse result = com.valravn.http.ProxyTunnel.sendOrFallback(api, request);
            ...
            while (followRedirects && result != null && result.response() != null && redirectCount < maxRedirects) {
                ...
                result = com.valravn.http.ProxyTunnel.sendOrFallback(api, redirReq);
                host = redirHost;
                port = redirPort;
                isHttps = redirHttps;
                redirectCount++;
