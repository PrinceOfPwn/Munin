...
import com.valravn.server.ApiServer;
import com.valravn.ui.ConfigTab;

import java.awt.GraphicsEnvironment;

public class ValravnExtension implements BurpExtension {

    private static final String EXTENSION_NAME = "Valravn MCP";
    ...
    private final com.valravn.store.FindingsStore findingsStore = new com.valravn.store.FindingsStore();
    ...
    public void initialize(MontoyaApi api) {
        ...
        int port = loadInt(api, "valravn.port", DEFAULT_PORT);
        String host = loadString(api, "valravn.host", DEFAULT_HOST);

        // Start API server first (creates SessionHandler + FindingsStore).
        // This is intentionally independent from Swing so Burp can run fully
        // headless in CI/labs while still exposing the Valravn REST surface.
        ...
        // the project blob.
        String savedFindings = loadString(api, "valravn.findings", "");
        ...
        findingsStore.setChangeListener(() ->
                saveString(api, "valravn.findings", findingsStore.exportJson()));

        // The dashboard is useful interactively but must never be a runtime
        // requirement. In headless mode the REST API remains fully active and
        // Munin can drive Burp without a display or startup wizard.
        if (GraphicsEnvironment.isHeadless()) {
            api.logging().logToOutput(
                EXTENSION_NAME + " running headless: dashboard UI disabled; REST API remains active"
            );
        } else {
        ...
        api.extension().registerUnloadingHandler(() -> {
            ...
            com.valravn.collaborator.CollaboratorPool.reset();
        ...
        synchronized (serverLifecycleLock) {
            ...
            try {
                ...
                api.logging().logToOutput("Proxy tunnel → "
                    + com.valravn.http.ProxyTunnel.BURP_PROXY_HOST + ":"
                    + com.valravn.http.ProxyTunnel.BURP_PROXY_PORT
                    + " (override with env BURP_PROXY_HOST/PORT or -Dvalravn.proxy.{host,port})");
            } catch (Exception e) {
    ...
    void restartServer(String newHost, int newPort) {
        synchronized (serverLifecycleLock) {
            ...
            saveString(api, "valravn.host", newHost);
            saveInt(api, "valravn.port", newPort);
        ...
        try {
            ...
            while (resources.hasMoreElements()) {
                try (var is = resources.nextElement().openStream()) {
                    ...
                    if (ver != null && title != null && title.contains("Valravn")) {
                        return ver;
                    }
                }
            }
    ...
    private void checkJvm(MontoyaApi api) {
        ...
        if (major < 21) {
            api.logging().logToError(
                "WARNING: Detected Java " + spec + ". Valravn MCP requires Java 21 or newer. "
    ...
    private static void saveInt(MontoyaApi api, String key, int value) {
        try {
            ...
            api.logging().logToError("[Valravn] persistence write failed for '" + key + "': " + e.getMessage());
        }
    }
