...
def _lookup_finding_id(finding_id: str) -> tuple[bool, str]:
    """Resolve a finding ID across every .valravn-intel domain.
...
def register(mcp: FastMCP):
    ...
    async def annotate_request(
        ...
        colors require either a `finding_id` that resolves in .valravn-intel, or
        ...
        if finding_id:
            ...
            if not known:
                return (
                    f"Refused: finding_id={finding_id!r} does not exist in .valravn-intel. "
