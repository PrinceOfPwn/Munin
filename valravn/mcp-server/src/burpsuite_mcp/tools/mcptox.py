...
must accept terms at 4open.science before download); this harness:
  ...
  1. Detects whether the operator has the corpus locally (`MCPTOX_CORPUS_DIR`
     env var OR `~/.valravn/mcptox/corpus/`).
  2. If absent, runs the in-tree heuristic audit pattern using Valravn's
...
Without the corpus, the harness still ships a useful in-tree audit covering:
  ...
  - Tool-name shadow: scan tool list for names that match Valravn's known
...
def _corpus_dir() -> Path | None:
    ...
    if env:
        ...
        if p.is_dir():
            return p
    default = Path.home() / ".valravn" / "mcptox" / "corpus"
    if default.is_dir():
        return default
    return None
