...
def register(mcp: FastMCP) -> None:
    ...
    async def rank_attack_targets(
        ...
        Reads from `.valravn-intel/<domain>/endpoints.json` (saved by
    ...
    async def find_targets_for_class(
        ...
        Joins `.valravn-intel/<domain>/endpoints.json` with the parameter risk
        ...
        if not domain:
            return {
                "vuln_class": cls,
                "targets": [],
                "note": "domain required (reads .valravn-intel/<domain>/endpoints.json)",
            }
