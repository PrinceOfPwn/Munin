...
def register(mcp: FastMCP):
    ...
    async def save_finding(
        ...
        Args:
            ...
            domain: Target domain for persistent .valravn-intel storage.
        ...
        # we MUST NOT persist anything to .valravn-intel/findings.json. Otherwise
        ...
        return (
            ...
            f"  Burp ID: {burp_id}\n"
            f"  Location: .valravn-intel/{_sanitized(resolved_domain)}/findings.json"
        )

    @mcp.tool()
    async def hydrate_burp_findings(domain: str = "all", include_suspected: bool = False) -> str:
        """Re-populate Burp's in-memory Findings tab from persisted .valravn-intel findings. Use after extension reload.

        Args:
            domain: Specific domain to hydrate, or 'all' for every .valravn-intel domain
        ...
        if not targets:
            return f"No findings.json found for {domain!r} under .valravn-intel/. Nothing to hydrate."
        ...
        if gate_errors:
            ...
            lines.append("")
            lines.append("Persistent .valravn-intel store unchanged. Re-capture the underlying")
    ...
    async def mark_finding_false_positive(
        ...
        Args:
            ...
            domain:     Target domain (where the finding lives in .valravn-intel)
    ...
    async def prune_findings(
        ...
        """Drop non-value findings from .valravn-intel/<domain>/findings.json and
