...
_VERIFY_HINTS: dict[str, list[str]] = {
    ...
    "deserialization": ["uid=", "groups=", "valravn-deserial-marker"],
...
def _raw_response(resp: dict[str, Any]) -> bytes:
    ...
    if len(body_bytes) > 65536:
        body_bytes = body_bytes[:65536] + b"\n... [truncated by Valravn poc_bundle: original > 64KB]"
...
def _verify_py(finding: dict, req: dict) -> str:
    ...
    return f'''"""verify.py — Valravn PoC verification.
...
def register(mcp: FastMCP):
    ...
    async def export_poc_bundle(
        ...
        `.valravn-intel/<domain>/artifacts/poc/poc-<finding_id>.tar.gz` unless output_dir given.

        Args:
            domain: target domain (used for .valravn-intel path resolution)
            finding_id: saved-finding ID
            output_dir: optional output directory (default .valravn-intel/<domain>/artifacts/poc/)
        """
    ...
    async def export_proof_capsule(
        ...
        Layout: `.valravn-intel/<domain>/artifacts/poc/<finding_id>/capsule/`
