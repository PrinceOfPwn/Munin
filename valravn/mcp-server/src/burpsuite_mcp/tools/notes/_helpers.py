...
def _intel_dir() -> Path:
    """Resolve the .valravn-intel directory at call time (cwd may change)."""
    return Path.cwd() / ".valravn-intel"
...
async def _hard_delete_finding(domain: str, finding: dict) -> tuple[bool, str]:
    """Remove a finding from .valravn-intel/<domain>/findings.json AND from Burp's
