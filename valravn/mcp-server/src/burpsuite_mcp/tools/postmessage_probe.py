...
JSON.stringify((() => {
  ...
  try { return window.__valravn_pm_handlers__ || []; } catch (_) { return []; }
})())
""".strip()
...
_MALICIOUS_PAYLOADS = [
    ...
    {"setHTML": "<img src=x onerror=__valravn_canary__=1>"},
    {"eval": "window.__valravn_eval_canary__=1"},
...
def register(mcp: FastMCP) -> None:
    ...
    async def probe_postmessage_listeners(  # cost: low (4-8 browser ops)
        ...
        canary_check = await client.post(
            "/api/browser/execute_js",
            json={"script": (
                "JSON.stringify({eval_canary: !!window.__valravn_eval_canary__,"
                " img_canary: !!window.__valravn_canary__})"
            )},
        )
