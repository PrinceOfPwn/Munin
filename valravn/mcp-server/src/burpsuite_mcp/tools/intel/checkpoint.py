...
Valravn persists coverage / findings / fingerprint per domain, but the *task
...
state in one read instead of scraping.
    ...
    .valravn-intel/<domain>/checkpoint.json
    {
      "domain": "example.com",
...
def register(mcp: FastMCP) -> None:
    ...
    async def write_checkpoint(
        ...
        Writes .valravn-intel/<domain>/checkpoint.json — the single durable record of
    ...
    async def load_checkpoint(domain: str) -> str:
        ...
        Reads .valravn-intel/<domain>/checkpoint.json and returns a one-glance summary:
