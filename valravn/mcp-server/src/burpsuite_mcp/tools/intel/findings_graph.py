"""build_findings_graph — cross-engagement typed-edge graph over .valravn-intel/.

Walks every `.valravn-intel/<domain>/findings.json` and emits a graph of typed
...
`.valravn-intel/_graph/graph.json` for programmatic re-read.
"""
...
def register(mcp: FastMCP):
    ...
    async def build_findings_graph(
        ...
        if not intel_root.exists():
            return "No .valravn-intel/ directory yet — nothing to graph."
