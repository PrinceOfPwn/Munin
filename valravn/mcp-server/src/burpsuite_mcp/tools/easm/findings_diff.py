"""findings_diff — scan-vs-scan delta over .valravn-intel/<domain>/findings.json snapshots.
...
on read into .valravn-intel/<domain>/_snapshots/findings-<iso>.json so the
...
def register(mcp: FastMCP) -> None:
    ...
    async def findings_diff(
        ...
        if not snaps and current != "live":
            return f"No snapshots in .valravn-intel/{domain}/_snapshots/."
    ...
    async def scope_targets_to_diff(
        ...
        Args:
            domain: target domain (slug) — reads .valravn-intel/<domain>/endpoints.json.
    ...
    async def archive_findings_snapshot(domain: str) -> str:
        ...
        if p is None:
            return f"No findings.json in .valravn-intel/{domain}/ to archive."
        return f"Archived: {p}"
