...
Valravn needs a published score. Without one, "leading AI security copilot"
is a claim, not a fact. XBOW: 75/104 XBOWBench. Strix: 100/104. PentestGPT:
86.5%. Pentest-R1: 24.2% AutoPenBench. These are public numbers; Valravn's
is currently zero.
...
Two wrappers:
  ...
  - run_autopenbench(challenge_id, timeout) — wraps the AutoPenBench docker
    ...
    challenge; this tool fires Valravn's grow-agent loop at it and records
    pass/fail to .valravn-intel/_bench/<benchmark>/<challenge>.json.
...
a README badge. We deliberately do NOT auto-tune Valravn for benchmark — that's
...
def register(mcp: FastMCP) -> None:
    ...
    async def run_autopenbench(
        ...
        graded by exact-flag match. This tool:
          ...
          3. Tool reads grow-agent's submitted flag from .valravn-intel/_bench/
             <challenge_id>/flag.txt and compares to challenge expected.
          4. Records pass/fail + duration + Valravn tool calls.
    ...
    async def xbow_pull_benchmarks(
        ...
        Default target_dir: ~/.valravn/benchmarks/xbow/
        ...
        Args:
            target_dir: where to clone. Empty -> ~/.valravn/benchmarks/xbow.
        """
        from pathlib import Path as _P
        if not _check_tool("git"):
            return {"error": "git not installed"}
        dest = _P(target_dir).expanduser() if target_dir else (
            _P.home() / ".valravn" / "benchmarks" / "xbow"
        )
    ...
    async def run_xbow_bench(
        ...
        if not challenge_path:
            for root in (
                _P.home() / ".valravn" / "benchmarks" / "xbow" / "benchmarks",
    ...
    async def summarize_benchmarks() -> dict:
        """Summarise all recorded benchmark runs under .valravn-intel/_bench/.
