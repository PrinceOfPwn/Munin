...
class MsfCheckToolTest(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="valravn-msf-"))
        self.prev = Path.cwd()
        os.chdir(self.tmp)
    ...
    async def test_vulnerable_verdict(self):
        ...
        audit = self.tmp / ".valravn-intel" / "_audit.log"
    ...
    async def test_out_of_scope_logged(self):
        ...
        audit = (self.tmp / ".valravn-intel" / "_audit.log").read_text().strip().splitlines()
...
class MsfExploitToolTest(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        self.tmp = Path(tempfile.mkdtemp(prefix="valravn-msf-exp-"))
        self.prev = Path.cwd()
        os.chdir(self.tmp)
