...
class VariantGeneratorTest(unittest.TestCase):
    ...
    def test_rsc_yields_at_least_five_shapes(self):
        v = cvp._rsc_variants("baseline-__CANARY__", "VALRAVN-DEADBEEF", "abc123")
    ...
    def test_rsc_canary_substituted_into_baseline(self):
        v = cvp._rsc_variants("body=__CANARY__", "VALRAVN-AAAA", "")
        baseline = [x for x in v if x["label"] == "rsc.baseline_with_headers"][0]
        self.assertIn("VALRAVN-AAAA", baseline["body"])
    ...
    def test_nextjs_cache_variants_use_known_headers(self):
        v = cvp._nextjs_cache_variants("", "VALRAVN-FEED", "")
        labels = [x["label"] for x in v]
        ...
        self.assertIn("VALRAVN-FEED", rm["headers"]["x-now-route-matches"])

    def test_trpc_variants_target_proto_and_constructor(self):
        v = cvp._trpc_variants("", "VALRAVN-BEEF", "")
        labels = [x["label"] for x in v]
    ...
    def test_proto_variants_cover_canonical_sinks(self):
        v = cvp._proto_variants("", "VALRAVN-CAFE", "")
        labels = [x["label"] for x in v]
    ...
    def test_generic_requires_baseline(self):
        ...
        v = cvp._generic_variants("foo=__CANARY__", "VALRAVN-1234", "")
...
class ScoringTest(unittest.TestCase):
    ...
    def test_canary_in_body_is_confirmed(self):
        verdict, conf, _ = cvp._score_response(
            "react_server_components", "VALRAVN-DEAD", 200, "",
            "Hello VALRAVN-DEAD world")
        self.assertEqual(verdict, "CONFIRMED")
        self.assertGreaterEqual(conf, 0.85)

    def test_canary_in_headers_is_confirmed(self):
        verdict, conf, _ = cvp._score_response(
            "react_server_components", "VALRAVN-BEEF", 200,
            "X-Echo: VALRAVN-BEEF", "")
        self.assertEqual(verdict, "CONFIRMED")

    def test_rsc_marker_with_500_is_suspected(self):
        verdict, conf, _ = cvp._score_response(
            "react_server_components", "VALRAVN-CAFE", 500, "",
    ...
    def test_no_marker_no_canary_is_failed(self):
        verdict, conf, _ = cvp._score_response(
            "react_server_components", "VALRAVN-ZZZZ", 404, "",
            "Not Found")
    ...
    def test_first_confirmed_short_circuits(self):
        ...
        async def fake_post(path, json):
            ...
            if call_count["n"] == 1:
                return {
                    "status_code": 200,
                    "response_body": "VALRAVN-AAAAAAAA echoed back",
                    "response_headers": "",
                    "proxy_index": 42,
                }
        ...
        with patch.object(cvp, "_canary", return_value="VALRAVN-AAAAAAAA"), \
