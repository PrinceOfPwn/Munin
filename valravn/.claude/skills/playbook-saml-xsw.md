---
description: SAML XML 签名包装 (XSW) 攻击——依 Somorovsky-Mainka-Schwenk 研究 8 种标准变体。手工构造 XML payload。当目标用 SAML 2.0 (经 Salesforce / Okta / Ping / OneLogin / ADFS / Shibboleth 之 SSO) 时载入。
globs:
---
...
This is the most operator-driven class in Valravn — no auto_probe coverage. Manual XML construction required. KB `saml_xsw` (W1) is operator-built reference; this playbook is the workflow.
...
- Burp SAML Raider extension — operator UI tool for XSW (binary extension, not Valravn-integrated)
