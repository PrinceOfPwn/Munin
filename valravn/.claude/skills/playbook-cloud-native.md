...
description: 对 AWS / GCP / Azure 托管应用的 web 利用。当目标暴露云原生组件 (Cognito token、S3/GCS/Blob URL、Lambda Function URL、App Service、Cloud Run、AWS SDK 错误、JS bundle 中 IAM 名) 时用。涵盖 IMDSv1/v2 SSRF、云存储错配、联邦身份漏洞、云专属 JWT 滥用——仅 web 应用技术。
...
Valravn is web-app focused. For these you need separate tooling:
