...
description: 动态移动插桩——Frida (iOS + Android)、adb (仅 Android)、objection。绕 SSL pinning / root-JB 检测令 Burp 见流量，运行时 hook crypto/HMAC/storage，滥用 Android exported 组件与 deep link，dump iOS keychain。流量既通即交 playbook-mobile-backend.md。仅动态，无静态反编译。
