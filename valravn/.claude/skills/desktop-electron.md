---
description: 桌面应用猎杀——Electron / Tauri / WebView2。静态二进制审视 + IPC fuzz + 代理流量截获。当目标为桌面二进制时载入。
globs:
---
...
- `@electron/fuses` (npx)
- Valravn existing: `run_trufflehog`, `run_gitleaks`, `match_replace`, `concurrent_requests`, `kev_epss_enrich`
