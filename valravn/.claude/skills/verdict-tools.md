---
description: 如何消费 Valravn 由 52+ 评估工具返回之 VerdictResult schema，及如何将新工具接入该 schema。当重构工具输出、构建 agent 循环、或读测试工具响应时载入。
globs:
---
...
**43 testing tools** return VerdictResult dict. All assessment-class tools converted. Remaining string-return tools fall into three intentional categories (see "When NOT to return a verdict"). See `MEMORY.md` `Valravn — W{N}` entries for the rolling count.
