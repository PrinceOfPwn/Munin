---
description: 移动应用 DAST 流程——OWASP MASTG / MASVS 映射。Frida + adb + Burp 代理。当范围内有 Android/iOS 应用时载入。
globs:
---

# Mobile MASTG Workflow

Use when: target has an Android/iOS app in scope (APK / IPA / store URL), or a web finding points at a mobile-specific surface (deep link, universal link, WebView bridge). Pair with `mobile-dynamic-agent`. NOT a substitute for static analysis — Valravn mobile is **supporter tier**, not focus.
...
## MASTG → Valravn mapping

| MASVS control      | MASTG test family        | Valravn surface                                |
|--------------------|--------------------------|------------------------------------------------|
...
Use `mobile_adb_pack(cmd_id, args)` for formatted commands. Valravn does NOT execute adb — operator runs on their authorized device.
...
- `.valravn-intel/<pkg>/manifest.json` — parsed AndroidManifest exports
- `.valravn-intel/<pkg>/deeplinks.json` — `{scheme, host, path, activity}[]`
- `.valravn-intel/<pkg>/webview_bridges.json` — exposed JS interface methods
- `.valravn-intel/<pkg>/endpoints.json` — backend API base URLs harvested from traffic
