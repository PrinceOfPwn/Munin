...
description: 生成平台特定赏金报告（HackerOne、Bugcrowd、Intigriti、Immunefi）。发现 `confirmed` 且 `assess_finding` 返 `REPORT` 后用。输出遵 PTES + OWASP WSTG 布局带 CVSS 4.0。
---

# 报告模板

## Pre-flight 门（Rule 28）

`generate_report` 或 `format_finding_for_platform` 前：

1. 发现有 `status='confirmed'`（否则排除）
2. `assess_finding` 返 `REPORT`（否则上游门拦截）
3. Step-0 replay 过（`verify-finding.md`）
4. 假阳性 → 设 `status='likely_false_positive'`。下次 `generate_report` 硬删。无跟踪、无墓碑。

`generate_report` 若言 "No reportable findings"，你啥都没确认——去验证，别跟门争。

## 规范发现布局（PTES §7 + OWASP WSTG）

每个发现——无论平台——须按此序含这些段。MCP `_build_finding_section` 在 `finding` dict 有正确 key 时已发此结构。

| 段 | Source key | 用途 |
|---|---|---|
| Classification | `vuln_type` `cwe` `owasp` `cvss_vector` `severity` `confidence` | Triager 按类筛 |
| Context | `context` | 此端点做什么、谁触达、为何重要 |
| Vulnerability | `description` | 通俗 bug 描述 |
| Attack Walkthrough | `attack_walkthrough` (list[str]) | 端到端利用：发现 → 触发 → 控制 → 影响 |
| Impact | `impact` | 具体后果（数据、访问、钱、账户控制） |
| Escalation Path | `escalation` `chain_with` | 如何链成更高影响 bug（ATO、RCE、横移） |
| Proof of Concept | `poc_request` (dict) | 精确 HTTP 请求——可复制复现 |
| Steps to Reproduce | `reproduction_steps` (list[str]) | 冷启步骤，triager 5 分钟内可跟 |
| Evidence | `evidence` `evidence_text` `reproductions` | Logger index、Collaborator ID、响应摘录、replay 表 |
| Remediation | `remediation` (list[str]) | 防御者具体修复指引 |
| References | `references` (list[str]) | CWE、OWASP、CVE、厂商公告、ATT&CK |

## 通用规则

1. **标题：** `[Bug Class] in [Component] allows [actor] to [impact]`
2. **影响优先** 在摘要——攻击者做**什么**，非 bug 如何运作
3. **<600 字** 主体（triager 读数百报告）
4. **复现须冷启 <5 分钟可工作**
5. **每报告一发现** —— 仅链时打包
6. **Severity 诚实** —— Rule 21。绝不虚报。

## CVSS 4.0 —— 用计算器

计算器：https://nvd.nist.gov/vuln-metrics/cvss/v4-calculator

CVSS v4 指标（Scope 移除；由 Vulnerable / Subsequent System 轴替代）：

| 指标 | 取值 | 注 |
|---|---|---|
| AV — Attack Vector | N=Network, A=Adjacent, L=Local, P=Physical | 多数 web 漏洞 = N |
| AC — Attack Complexity | L=Low, H=High | 需 race / 特定配置时 H |
| AT — Attack Requirements | N=None, P=Present | 需攻击者控外条件时 P |
| PR — Privileges Required | N=None, L=Low, H=High | N=unauth, L=任何用户, H=admin |
| UI — User Interaction | N=None, P=Passive, A=Active | XSS/CSRF=A；IDOR/SQLi=N |
| VC, VI, VA — Vulnerable System Confidentiality / Integrity / Availability | H/L/N | 对脆弱组件直接影响 |
| SC, SI, SA — Subsequent System impact | H/L/N | 传播到其他组件的影响 |

Severity 档（CVSS-BR base）：None 0.0 / Low 0.1–3.9 / Medium 4.0–6.9 / High 7.0–8.9 / Critical 9.0–10.0。

**常见起始向量**（按目标换 AT/PR/UI/SC/SI/SA）：
...
H1 提示：用其 severity 分类；引项目策略定 scope；CWE 必填；remediation 可选。
...
Bugcrowd 提示：VRT 映射必填；P1–P5（P1=Critical）；偏好 raw HTTP 流量。
...
Intigriti 提示：重 CVSS；查披露报告避 dup；req 和 resp 都要。
...
Immunefi 提示：remediation 必填；severity 按风险资金（Critical $10M+、High $1M+、Medium $100K+）。
...
- [ ] 冷启复现 <5 分钟
- [ ] 无假设知识
- [ ] 影响具体，非 "attacker can do bad things"
- [ ] CVSS 4.0 诚实，不虚报
- [ ] 含 raw req + resp
- [ ] 截图无敏感数据泄露
- [ ] 对当前生产测试过
- [ ] 对照项目策略双查 scope + 排除 vuln-types
- [ ] 每报告一发现（或链清晰标注）

## Severity 虚报 —— 勿做

- 无 ATO 链的 Reflected XSS 标 "Critical"
- 称 "RCE from SQLi" 但仅 error-based 证明
- 无 token 盗窃链的 open redirect 标 "High"
- 无证明数据外泄的 CORS 配错标 "Critical"
- "Could lead to" 但无证明确实
- PoC 显示有限影响时用最坏情况 CVSS

## 交叉引用

- **发现生命周期：** `verify-finding.md`
- **NEVER SUBMIT 列表 + 7 问门：** `.claude/rules/hunting.md`
- **链低成高：** `chain-findings.md`
- **CVSS 4.0 计算器：** https://nvd.nist.gov/vuln-metrics/cvss/v4-calculator

## 按漏洞类报告骨架（W16-W17 深挖伴侣）

下面骨架为每个主要类预填规范布局，匹配对应深挖 playbook 中的证据阶梯。复制、填 `<placeholders>`、提交。

### SSRF（按 `playbook-ssrf-deep-dive.md`）
...
### IDOR / BOLA（按 `playbook-idor-bola.md`）
...
### JWT（按 `playbook-jwt-deep-dive.md`）
...
### OAuth / OIDC 流攻击（按 `playbook-oauth-flow-attacks.md`）
...
### HTTP Request Smuggling（按 `playbook-request-smuggling.md`）
...
### Prototype Pollution（按 `playbook-prototype-pollution.md`）
...
## 按漏洞类报告骨架（W19-W20 深挖伴侣）

### Insecure Deserialization（按 `playbook-deserialization.md`）
...
### SAML XSW（按 `playbook-saml-xsw.md`）
...
### GraphQL（按 `playbook-graphql-deep-dive.md`）
...
### WebSocket 攻击（按 `playbook-websocket-attacks.md`）
...
### Web Cache Deception（按 `playbook-cache-deception.md`）
...
### Server Action / RSC（按 `playbook-server-action-rsc.md`）
...
| Other | 上方规范布局 | `hunt.md` / `verify-finding.md` |
