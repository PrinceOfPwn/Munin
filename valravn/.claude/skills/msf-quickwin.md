---
name: msf-quickwin
description: 已识 CVE / 指纹服务 / 扫描器检出漏洞时按需发 Metasploit exploit——用审计过的 MSF 模块，非从零造 payload。
---
...
| Modern attack class (RSC Flight, OAuth chains, GraphQL drift, CSPP/SSPP, MCP tool poisoning) | NO — Valravn KB + custom flow; MSF lags here |
...
6. **Save evidence.** Successful exploit returns `session_opened:true` + raw msfconsole output. Save via `save_finding(...)` with `evidence` linking the MSF run + `.valravn-intel/_audit.log` entry. Severity per real impact (CRITICAL for confirmed RCE with shell).
...
- **Rule 1** — RHOSTS / RHOST options scope-checked via Burp extension before fire. Out-of-scope targets logged to `.valravn-intel/_audit.log` (operator-mode) or refused (strict-mode).
...
## Sequencing with the rest of Valravn
...
- Authentication / authorization flaws — they need session state Valravn manages
- Modern web-app classes (RSC, GraphQL, OAuth flows) — Valravn's KB + oauth_flow_simulator + W22 tools
- Business-logic vulnerabilities — these are target-specific
- Information disclosure / IDOR — `test_auth_matrix` + `compare_auth_states`

Use MSF for **network-service-level CVE-tagged exploits**. Use Valravn's native tooling for **application-logic** + **modern web-class** + **authenticated flows**.
