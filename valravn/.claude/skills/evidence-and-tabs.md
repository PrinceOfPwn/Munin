...
description: 真渗透者如何用 Burp 诸面——Proxy 历史搜、Repeater 标签、Intruder、Organizer、标注。需 (a) 找已截证据 / (b) 迭代已截请求 / (c) 暴力/刷量/限速测 / (d) 为报告加书签时用。替代 "默认 curl" 反模式。
---
...
2. build_target_header_profile(domain)         # picks the most browser-like captured request,
                                               ...
                                               # saves to .valravn-intel/<domain>/profile.json
