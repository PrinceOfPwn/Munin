---
description: 原型污染——CSPP (客户端) vs SSPP (服务端)。Express / Fastify / Hapi sink。CSPP→DOM XSS。SSPP→RCE / 鉴权绕过。当 JSON body 合并 + 嵌套对象被接受时载入。
globs:
---
...
3. **Inject canary** — `?__proto__[valravnCanary]=1` or fragment `#/route?__proto__[valravnCanary]=1`.
4. **Verify pollution** — open devtools console: `Object.prototype.valravnCanary` should be `1`.
...
`test_dom_sinks(url, source_kinds=['fragment','fragment_kv','query'], cspp_known_keys=['valravnCanary','isAdmin','ALLOWED_TAGS','toString'])` — DOM probe (W9 VerdictResult). Reports which sinks reflect the canary AND whether prototype pollution propagated to known gadgets.
...
2. **Submit canary** — `POST /api/settings` with body `{"__proto__":{"valravnCanary":"polluted"}}`.
