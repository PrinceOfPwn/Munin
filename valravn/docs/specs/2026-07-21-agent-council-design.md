...
- **Valravn tools are deterministic.** `debate_triage` deliberately hands the *calling* model a scaffold rather than running an LLM jury. Voting follows the same model: either the convener reasons through structured positions, or real voter subagents return votes that a deterministic tool tallies. No hidden juries.
