...
Valravn's **per-attack-class depth and evidence provenance already beat commercial DAST**. The competitive frontier — XBOW, Horizon3 NodeZero, Terra, Alias CAI — leads on four axes Valravn lags:
...
3. **KB-freshness speed** — Nuclei-AI ships templates hours after disclosure; Valravn is on a weekly hand-authored cadence.
4. **Per-decision token cost** — competitors amortize with fine-tuned/distilled models.

Every item below **builds on primitives Valravn already ships**. No green-field.

## Moats — defend, do not trade away for autonomy

- **Evidence provenance.** Every finding is Burp-captured + `logger_index`-replayable. XBOW's own research documents LLMs fabricating proof; Valravn's replay + `VerdictResult` + NEVER-SUBMIT gate is the structural counter. This is the strongest moat — every autonomy feature must preserve it.
