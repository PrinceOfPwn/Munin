...
**Scope:** Close the four axes where the autonomous-pentester frontier (XBOW, Horizon3 NodeZero, Terra, Alias CAI) leads — autonomy loop, validation rigor, KB-freshness speed — WITHOUT abandoning Valravn's evidence-provenance and operator-gate moats. Depends conceptually on Spec E (progress ledger, cost-in-loop) and the agent-council spec (Tier-2 panel).

## Problem

Valravn's coverage and evidence discipline beat commercial DAST, but three structural gaps define the frontier:
...
- **Gap:** Nuclei `-ai` / PDCP generate templates from natural language; Valravn's KB is hand-authored JSON on a weekly cadence.
