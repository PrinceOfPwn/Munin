# Valravn v1.0 Milestone — Plan

**Date:** 2026-05-24
**Goal:** Rebrand to **Valravn**, close the gaps identified in the competitive analysis (CAI / Noir / pentest-ai / Burp DAST AI), ship as v1.0.

## Architecture

```
Claude Code  <- stdio MCP ->  valravn-mcp (Python)  <- HTTP ->  valravn-burp-ext (Java)  <- Montoya ->  Burp Suite
                                  |
                                  +- subprocess: opengrep, gitleaks, trufflehog, git-dumper, Noir
                                  |
                                  +- persistent: .valravn-intel/<domain>/
```

## Rename strategy — SOFT RENAME for v1.0

External identifiers (PyPI name, JAR name, display strings) become "Valravn". Internal Python package directory `burpsuite_mcp/` stays unchanged for v1.0 to avoid touching ~200 import statements and tests. Hard package rename deferred to v1.1 with a re-export shim. Result: users install `pip install valravn-mcp`, drop `valravn-burp-ext-1.0.0.jar` into Burp, see "Valravn" everywhere user-visible. Trademark exposure resolved.
...
- `.mcp.json` — server name (`valravn`), keep old MCP server name as alias if dual-keyed
- `MEMORY.md` — note rebrand fact

**NOT touched (deferred to v1.1):**
- `mcp-server/src/burpsuite_mcp/` directory and all its imports
- Java package `com.valravn.*` — Java packages stay, only the JAR artifact name changes
- `.valravn-intel/` directory structure (operator data, agnostic to product name)
...
| `dump_exposed_git` | `tools/secrets/git_dumper.py` (new) | Wraps git-dumper, outputs to `.valravn-intel/<domain>/git_dump/` |
...
| Noir OpenAPI ingest | extend `tools/scope/import_scope.py` | New `--noir-json` flag; merges endpoint metadata into `.valravn-intel/<domain>/endpoints.json` with `source: 'noir'` and `guards/sinks` arrays |
...
- `mvn clean package` produces `valravn-burp-ext-1.0.0.jar`
...
- README + skill.json + pom + pyproject + CLAUDE.md show "Valravn" / "valravn"
- Public-facing tagline: "Valravn — agentic DAST orchestrator for Burp Suite"
