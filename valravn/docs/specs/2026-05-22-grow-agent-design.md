...
Inspired by Hermes (Nous Research): persistent memory + auto-generated skills + isolated subagents + zero-context pipelines. This project already has the persistent memory (`.valravn-intel/`) and the subagent roster (`AGENTS.md`). grow-agent is the missing orchestrator that ties them together.
...
grow-agent (one-shot run, on-demand)
  ...
  │   ├─► Proposal write — .valravn-intel/_growth/proposals/<ts>-*.json
  ...
  │
  └─► .valravn-intel/_growth/patterns.json [auto-write on confirmed finding]
```
...
New directory `.valravn-intel/_growth/` (project-relative, gitignored):
...
|---|---|---|---|
| `domain` | string | required | Target slug (matches `.valravn-intel/<domain>/`) |
...
CHECKPOINT
  ...
  Append round audit line to .valravn-intel/<domain>/notes.md:
...
| Output | Path | Mode |
|---|---|---|
| Intel updates | `.valravn-intel/<domain>/{profile,endpoints,coverage,findings,fingerprint,patterns,notes}.json` | Existing tool layer (save_target_intel) |
| Round audit log | `.valravn-intel/<domain>/notes.md` (append) | grow-agent |
| Pattern aggregate | `.valravn-intel/_growth/patterns.json` | grow-agent (auto-write) |
| Proposals | `.valravn-intel/_growth/proposals/<ts>-*.{json,md}` | grow-agent (propose-only) |
...
- After a session, `.valravn-intel/_growth/patterns.json` reflects confirmed findings
...
`.claude/agents/grow-agent.md` and 9 sub-agent files are pure markdown. Rollback = `git revert <commit>`. No code, no schema, no DB migrations. `.valravn-intel/_growth/` directory is gitignored — deleting it removes all state.
