...
- **Design:** Valravn caps concurrency at 6 but has **no floor**. Add a routing rule to `grow-agent` DECIDE + `dispatch-agents.md`: trivial → direct tool, no agent; single class on ≤3 endpoints → 1 worker; broad coverage → partition to ≤6. Tie the decision to `check_cost_budget`.
