...
The KB is hand-authored on a weekly `W##` cadence. Between drops, high-value public techniques go untested. The 2026-H2 harvest (PortSwigger Top-10-of-2025 published Feb 2026, plus mid-2026 advisories) contains four **P0** techniques Valravn cannot currently detect and six **P1** items, none in the current 150-file KB.
...
- **Valravn status:** has schema-drift + stdio-shell-meta + tool-poisoning KB, but NO invisible-unicode detector. Distinct defect class.
...
- **Valravn status:** SSTI KBs cover engines but not the error-oracle method, and no Elixir engine.
...
- **Valravn status:** SSRF KBs lack the redirect-threshold-exhaustion leak.
...
- **Valravn status:** `nextjs_cache_poisoning.json` + `react_server_components.json` have W31-c CVEs (44578/27980) but not this trio.
...
| ID | Technique / CVE | Valravn form | Source |
|---|---|---|---|
