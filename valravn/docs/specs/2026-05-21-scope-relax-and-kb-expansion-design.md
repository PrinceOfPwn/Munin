...
- Mode persists at `.valravn-intel/_scope_mode.json` (survives sessions).
- `BaseHandler.requireInScope` checks mode first:
  - `strict` → current behavior unchanged
  - `operator` → `auditScope(url, tool, engagement)` appends to `.valravn-intel/_audit.log` (JSONL) and returns true
...
`.valravn-intel/_audit.log` (JSONL, always on regardless of mode):
...
- Cache resolved path to `.valravn-intel/_seclists_path.json`
...
Input sources:
- `.valravn-intel/<domain>/fingerprint.json` → tech stack
- `.valravn-intel/<domain>/endpoints.json` + sitemap + wayback → recon-derived path segments
...
Output: `.valravn-intel/<domain>/_wordlists/fuzz-<tier>.txt` — deduped, ordered (recon-derived first, tech-specific next, generic last).
