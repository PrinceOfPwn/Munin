...
**Architecture:** YAML-frontmatter agent files in `.claude/agents/` auto-discovered by Claude Code's `Agent` tool. grow-agent owns the session lifecycle and dispatches the 9 sub-agents by name. Storage in `.valravn-intel/_growth/` (gitignored).
...
**Files:**
- Create: `.valravn-intel/_growth/.gitkeep`
- Create: `.valravn-intel/_growth/proposals/.gitkeep`
- Modify: `.gitignore` (verify `.valravn-intel/` is already excluded — if so, no change)

- [ ] **Step 1: Verify `.gitignore` excludes `.valravn-intel/`**

Run: `grep -n burp-intel <repo-root>/.gitignore`
Expected: at least one match. If absent, append `.valravn-intel/` to `.gitignore`.

- [ ] **Step 2: Create `_growth/` and `_growth/proposals/` directories**

Run: `mkdir -p <repo-root>/.valravn-intel/_growth/proposals`
Expected: directories exist, gitignored.

- [ ] **Step 3: Verify on-disk layout**

Run: `ls -la <repo-root>/.valravn-intel/_growth/`
...
- `domain` (required) — slug matching `.valravn-intel/<domain>/`
...
CHECKPOINT:
  ...
  append to .valravn-intel/<domain>/notes.md:
...
- `mode="reflect"` — reads `.valravn-intel/<domain>/` + `_growth/patterns.json`; returns summary of prior session + uncovered gaps + promotion candidates. No writes.
...
```bash
cd <repo-root>
git add .claude/agents/grow-agent.md .valravn-intel/_growth/.gitkeep .valravn-intel/_growth/proposals/.gitkeep 2>/dev/null || true
git commit -m "feat(agent): add grow-agent session orchestrator with growth proposals"
```

(`.valravn-intel/` is gitignored; the `.gitkeep` files won't be added — `git add` will silently skip them. This is fine; the directories are created at runtime when needed.)
...
1. Load finding from `.valravn-intel/<domain>/findings.json`
...
6. Save the working bypass to `.valravn-intel/<domain>/notes.md` via `save_target_notes`
...
```bash
ls .valravn-intel/_growth/proposals/ 2>/dev/null || mkdir -p .valravn-intel/_growth/proposals/
...
- Storage: `.valravn-intel/_growth/{patterns.json, proposals/<ts>-*}` (gitignored)
...
- `.valravn-intel/_growth/proposals/` runtime-created when grow-agent first writes a proposal (gitignored)
