...
2. **Valravn's verification + export surface already exceeds commercial DAST.** No competitor reviewed beats its 7-question gate / NEVER-SUBMIT taxonomy / SARIF+JUnit+4-platform+CVSS4.0+KEV/EPSS export. Do not rebuild these. Adopt only what fills a real hole.
...
- **Loop guard:** abort/re-plan when the same tool+args fires ≥N times (source: PentAGI mentor @5 identical calls). Valravn has no runaway backstop.
...
This is a genuine build, not an adoption — no reviewed project ships the tooling, only references it. **Decision needed before starting:** does Valravn stay web/API/cloud-scoped, or expand into internal-network/AD? Recommend documenting as an explicit non-goal in README unless an engagement demands it.
...
- **Gap:** ARTEMIS — top human 13 bugs via business logic/chaining vs best agent 9; "~70% of critical web vulns live in business logic, the one class no agent detects reliably." Valravn's `test_business_logic` / `infer_business_invariants` / `capture_business_context` / Rule 27 are advisory — nothing enforces they ran. Recon gate (Rule 20a) is enforced; the business-logic pass is not.
...
- **Gap:** Invicti discovers APIs from source; Valravn only from crawl/proxy/JS. Grey/white-box hole.
...
- **Gap:** Burp 2026.6 continuously reprioritizes the audit queue, highest-value first. Valravn's `rank_attack_targets` / `risk_rank_endpoints` don't drive `auto_probe` order.
...
Source: `XiaomiMiMo/MiMo-Code` — checkpoint.md task-tree + independent goal/stop judge. Mapping showed Valravn covers roles / skill-discovery / memory distillation / structured per-agent status already; the two real gaps were durable task state and an independent completion check. Both file-based, no new deps, reuse existing `.valravn-intel/` + `business_logic_gate`.
...
- **Fix:** `write_checkpoint` / `load_checkpoint` → `.valravn-intel/<domain>/checkpoint.json` (phase, round, next_action, hierarchical task tree, open_threads). Merges by task id (field-level; a status flip never drops a title/note). `intel/checkpoint.py`. Wired: grow-agent Round 0 LOAD + CHECKPOINT step, resume.md step 1b.
- **Effort:** low. Done.
...
| Episodic action→outcome memory + selective recall + cred-redact | PentAGI, PentesterFlow | Avoid repeating dead-end probes; extends `.valravn-intel` intel |
...
- Docker-per-agent sandbox (Strix/PentAGI) — Valravn runs in Claude Code, not a server; sandbox is the harness's job.
- Vector-DB/Neo4j memory (PentAGI Graphiti) — over-engineered vs the file-based `.valravn-intel` intel that already works.
- Auto-fix / PR-patch generation (Strix) — out of a pentest tool's lane; scope creep.
- Push-time / CI leak-scan + frontmatter linter (ClaudeBrain, pentest-ai-agents) — Valravn is a local tool; engagement findings live in gitignored `.valravn-intel/` and are never pushed, so a pre-push gate solves a problem this project doesn't have.
```
