...
- **Commit identity:** `PrinceOfPwn` / `63230297+PrinceOfPwn@users.noreply.github.com` (already set). Never add an AI co-author. Commit prefixes: `perf(...)`, `feat(kb): ...`, `feat(probes): ...`.
- **Test command:** `cd mcp-server && uv run python -m unittest tests.<module> -v`.
- **Working dir for all paths below:** repo root `/home/kali/project/trc/valravn`. Python source root: `mcp-server/src/burpsuite_mcp/`. Tests: `mcp-server/tests/`.
...
Run:
```bash
cd /home/kali/project/trc/valravn/mcp-server
...
def _nextjs_middleware_variants(baseline: str, canary: str, action_id: str) -> list[dict[str, Any]]:
    ...
    variants.append({
        ...
        "headers": {"RSC": "1", "X-Valravn-Canary": canary}, "body": "",
    })
    ...
    variants.append({
        ...
        "headers": {"X-Valravn-Canary": canary}, "body": "",
    })
    return variants
```
