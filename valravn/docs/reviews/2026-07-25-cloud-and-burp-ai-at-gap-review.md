...
**Scope:** Can Valravn (a) do cloud-security pentest/red-team work per the 2026
...
Valravn is **already competitive-to-superior** on both axes. It is not missing a
...
Valravn analog for each:

| Burp AI feature | Valravn analog |
|---|---|
...
| Montoya AI API (BYO extension AI) | N/A — Valravn *is* the external agent |
| PortSwigger MCP server (transport bridge) | Valravn's Java ext + ~369 MCP tools |

**Where Valravn beats Burp AI:** model choice / offline, no 12-month credit
...
unattended autonomy; no advertised exploit-chaining. Valravn matches/beats on 7
...
equivalent — Valravn had `--paranoid/--normal/--aggressive` but no risk-tiered
...
**Already present in Valravn:** AND-composition across matchers + `not_word`
...
`debate_triage`). Valravn's FP stack was already mature.
...
computed marker round-trip) already exists in Valravn: probes embed canaries
(`VALRAVN-<hex>`, arithmetic `1337`=`1336+1`, SSTI `{{7*n}}`) and the resolved
