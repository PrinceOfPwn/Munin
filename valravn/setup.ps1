...
function Resolve-ValravnJar([string]$root) {
    $targetDir = Join-Path $root 'burp-extension\target'
    if (-not (Test-Path $targetDir)) { return $null }
    $hit = Get-ChildItem -Path $targetDir -Filter 'valravn-burp-ext-*.jar' -File -ErrorAction SilentlyContinue |
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { Fail "Maven build failed" }
        ...
        $jarPath = Resolve-ValravnJar $ScriptDir
...
Write-Host "  3. Verify: 'Valravn MCP started on port 8111' in Burp output"
