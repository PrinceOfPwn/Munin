#!/usr/bin/env bash
# Build the Valravn Burp extension and say — unambiguously — where the jar is.
#
...
echo "Building Valravn Burp extension (${EXT_DIR})"
...
ARTIFACT_ID="$(mvn -f "$EXT_DIR/pom.xml" -q -DforceStdout help:evaluate -Dexpression=project.artifactId 2>/dev/null || echo valravn-burp-ext)"
...
echo " Verify: the Burp output log prints 'Valravn MCP v${VERSION} started'"
echo "         and a 'Valravn MCP' suite tab appears."
echo "======================================================================"
